...
| Lifecycle   | `subagent-manager.ts`, `create-subagent-session.ts`, `subagent-session.ts`, `turn-limits.ts`, `subagent.ts`, `concurrency-limiter.ts`, `parent-snapshot.ts`, `child-lifecycle.ts`, `workspace.ts`, `usage.ts` | 10      | Spawn, abort, resume, concurrency admission, session assembly factory, born-complete turn loop, status state machine, per-subagent behavior, child-lifecycle events, workspace provider seam |
